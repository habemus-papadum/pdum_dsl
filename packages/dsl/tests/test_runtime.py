...
def test_a_kernel_actually_runs():
    f = reference(make_shader(0.3, 2.0))
    assert f(0.5) == 2.0  # d = 0.2 < 0.5 -> gain
    assert f(0.9) == 0.0  # d = 0.6 -> other branch


def test_plain_calls_refuse_the_oracle_is_spelled():
    """No device backends exist until the L4 era; a plain call must REFUSE,
    never silently interpret (200 §1.1)."""
    from pdum.dsl.registry import NoBackend

    f = make_shader(0.3, 2.0)
    with pytest.raises(NoBackend, match="spelled"):
        f(0.5)


def test_the_thesis_300_frames_one_compile(counters):
    reference(make_shader(0.0, 1.0))(0.5)  # frame 0: the one compile
    with no_compile():  # every further frame MUST be a hit — enforced, not hoped
        for i in range(1, 300):
            assert reference(make_shader(i * 0.001, 1.0 + i))(0.5) == (1.0 + i)
...
def test_arg_type_change_is_a_new_specialization(counters):
    f = reference(make_shader(0.0, 2.0))
    f(0.25)
...
def test_cold_key_under_no_compile_is_forbidden_and_named():
    f = reference(make_shader(-1.0, 5.0))
...
def test_identical_bodies_share_the_artifact(counters):
    ...
    assert reference(site_a(2.0))(3.0) == 6.03125
    assert reference(site_b(5.0))(3.0) == 15.03125
...
def test_pipelines_execute_fused(counters):
    ...
    assert reference(scale(3.0) | shift(1.0))(2.0) == 7.0
    assert reference(scale(3.0) | shift(1.0) | collect)(2.0) == 7.0
    with no_compile():  # new stage configs, same types: the pipe specialization holds
        assert reference(scale(4.0) | shift(2.0))(5.0) == 22.0


def test_value_gt_pipeline_refuses_unrouted():
    """``value > pipeline`` is the plain-call path: with no routed backend it
    refuses exactly like a plain call (the dispatcher is honest)."""
    from pdum.dsl.registry import NoBackend
    ...
    with pytest.raises(NoBackend, match="spelled"):
        2.0 > (scale(3.0) | scale(2.0))


def test_rendered_source_reads_the_staging_abi():
    f = make_shader(0.7, 9.0)
    reference(f)(0.5)
    key = DEFAULT.specializations.key_for(f, (DEFAULT.table.fingerprint(0.5),), ("reference", 1))
...
def test_generation_bump_recompiles_but_artifacts_survive(counters):
    f = reference(make_shader(0.1, 1.5))
    f(0.5)
...
def test_bare_registry_refuses_unknown_kind():
    """A hand-built Registry has an EMPTY kind vocabulary: dispatch refuses
    the kind before anything else (the F33 extraction fix)."""
    from pdum.dsl.ir import VerifyError

    bare = Registry()
    f = make_shader(0.0, 1.0)
    with pytest.raises(VerifyError, match="unknown kind"):
        bare.dispatch(f, (1.0,))
...
def test_lazy_branches_guard_then_divide():
    ...
    assert reference(guarded)(2.0) == 0.5
    assert reference(guarded)(0.0) == 0.0  # the eager renderer raised ZeroDivisionError here
    ...
    assert reference(make(0.5))(2.0) == 3.0
    assert reference(make(0.5))(-1.0) == -1.5
...
def test_per_kind_routing_with_a_real_route():
    """Routes map kinds to backends; a ROUTED kind dispatches plainly (that is
    what a real backend is); unrouted kinds refuse."""
    from dataclasses import replace

    from pdum.dsl import install as install_batteries

    reg = install_batteries(Registry())
    hits = []

    def spy_compile(source, name="kernel"):
        hits.append(name)
        return REFERENCE.compile(source, name)

    spy = replace(REFERENCE, name="spy", fp=("spy", 1), compile=spy_compile, code_for_op=dict(REFERENCE.code_for_op))
    reg.register_backend(spy, kinds=("spykind",))
    # intrinsics spelled onto "reference" at install; give the spy the same table
    reg.backends["spy"].code_for_op.update(reg.backends["reference"].code_for_op)

    @jit(kind="spykind")
    def k(x):
        return x + 1.0

    assert reg.dispatch(k, (1.0,)) == 2.0  # routed kind: plain dispatch through the spy
    assert hits == ["kernel"]

    from pdum.dsl.registry import NoBackend

    with pytest.raises(NoBackend):  # "device" has no route and no default here
        reg.dispatch(make_shader(0.0, 2.0), (0.25,))


def test_fresh_registry_via_install_seams():
    """Surface E is not a singleton in disguise: a hand-built Registry gets
    the same batteries through the explicit install() seam."""
    from pdum.dsl import install as install_batteries

    reg = install_batteries(Registry())
    f = make_shader(0.25, 4.0)
    assert reg.dispatch(f, (0.5,), backend="reference") == 4.0
