...
def test_fragment_role_arrives_with_its_domain():
    """Roles ship with their owning packages: a domain package registers its
    kind WITH a hint, and composition refusals quote it."""
    DEFAULT.register_kind(
        "test.fragment", hint="a fragment kernel maps pixel coordinates to a color; it ends a pipeline", terminal=True
    )

    @jit(kind="test.fragment")
    def frag(x):
        return x
    ...
    with pytest.raises(IncompatibleRoles, match="pixel coordinates"):
        Stage(frag) | Stage(dev)  # nothing may follow a terminal; the hint explains the domain
...
def test_unregistered_kind_is_loud():
    ...
    with pytest.raises(IncompatibleRoles, match="no kind registered"):
        add(1) | k
    DEFAULT.register_kind("audio_node")  # registering opens the kind, but no rule yet:
...
def test_apply_without_dispatcher_is_loud():
    previous, DEFAULT.dispatcher = DEFAULT.dispatcher, None  # batteries installed the real one
    try:
        with pytest.raises(NotYetExecutable):
            6 > (add(1) | mul(2))
    finally:
        DEFAULT.dispatcher = previous
...
def test_the_thesis_for_pipelines():
    ...
    prev, DEFAULT.dispatcher = DEFAULT.dispatcher, dummy
    try:
