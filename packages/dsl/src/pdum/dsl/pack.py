...
def legalize_params(plan: PackPlan, extra_legal: frozenset = frozenset()) -> Stage:
    ...
    def fire(b, m):
        ...
        if spec is None:
            ...
            if isinstance(node.type, Record):  # designed refusal (150 F14): the first
                raise VerifyError(  # path every record-curious author hits
                    f"a record value ({node.type!r}) is used whole where a scalar is needed — "
                    "records marshal by their fields: extract them (r.x), or return scalars; "
                    "whole-record results arrive with tensors-of-value-types (200 §S.3)"
                )
            raise VerifyError(
