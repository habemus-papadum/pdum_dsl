...
class FnType(Type):
    ...
    # Reserved (200 §S.4, landed at extraction while the identity vocabulary is
    # still being moved): the declared result type, None until a tier needs it
    # (the varyings interface check; fold-step diagnostics). None keeps every
    # pre-existing FnType identity unchanged.
    result: Type | None = None

    def __repr__(self) -> str:
        inner = ", ".join(repr(t) for t in self.env_types)
        suffix = f" -> {self.result!r}" if self.result is not None else ""
        return f"Fn<{self.template.label}>({inner}){suffix}"
