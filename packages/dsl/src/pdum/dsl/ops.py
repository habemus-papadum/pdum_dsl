...
def _arith(args, attrs, regions) -> Type:
    for a in args:  # designed refusal (150 F14): whole-record arithmetic must not
        if not isinstance(a, (Scalar, Vec)):  # crash later in legalization internals
            raise TypeError(
                f"core arithmetic works on scalars, not {a!r} — extract the record's fields "
                "(r.x * s.x), or define a method on the record for the operation"
            )
    if args[0] != args[1]:
