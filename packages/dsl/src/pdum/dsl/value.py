...
from .lower import MissingRule, fmt
from .types import Record, boolean, f64, i64
from .valuekind import typeof
...
def _assign(ctx, node):
    ...
    if isinstance(target, ast.Tuple) and all(isinstance(e, ast.Name) for e in target.elts):
        from .types import Tuple as TupleType
...
def _call(ctx, node):
    ...
    from .lower import NameFateError
