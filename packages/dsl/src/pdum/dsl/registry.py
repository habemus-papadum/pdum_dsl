...
backends, and the two cache tiers. Batteries install INTO it explicitly
(``pdum.dsl.install(registry)`` wires the value language, the pipe, and
the reference oracle); the engine never imports a satellite.
...
class NoBackend(RuntimeError):
    """Dispatch reached a kind with no routed backend. Oracle execution is
    always spelled: ``reference(f)(...)`` (design 200 §1.1)."""
...
class Registry:
    ...
    def __init__(self, table: KindTable = BUILTINS):
        ...
        self.routes: dict[str, str] = {}  # kind -> backend name; KNOWN-but-unrouted kinds use the default
        self.kinds: dict[str, str] = {}  # the VALIDATED kind vocabulary (200 §1.4): name -> hint
        self.terminal_kinds: set[str] = set()  # terminals (materializers) may only end a pipeline
        self.compositions: dict[tuple, str] = {}  # (op, left, right) -> semantics tag ("fuse", ...)
        self.dispatcher = None  # ``value > pipeline`` application hook (pipe.py)
        self.default_backend: str | None = None
        self.specializations = SpecializationCache()
        self.artifacts = ArtifactCache()

    def register_kind(self, name: str, *, hint: str = "", terminal: bool = False) -> None:
        """Admit ``name`` into the kind vocabulary. Dispatch REFUSES kinds
        never registered (the unknown-kind hole closed at extraction — 150
        F33). Role vocabularies ship with their owning packages."""
        self.kinds[name] = hint
        if terminal:
            self.terminal_kinds.add(name)

    def register_backend(self, backend: Backend, *, default: bool = False, kinds: tuple = ()) -> None:
        self.backends[backend.name] = backend
        for k in kinds:
            self.kinds.setdefault(k, f"registered by backend {backend.name!r}")
        self.routes.update(dict.fromkeys(kinds, backend.name))  # roles ship WITH their backends
        if default:
            # The default slot is claimed EXPLICITLY, never by arrival order:
            # a kinds-free registration silently becoming the default would
            # let plain calls reach the reference oracle unspelled (200 §1.1).
    ...
    def backend_for(self, kind: str) -> Backend:
        if (name := self.routes.get(kind, self.default_backend)) is None:
            raise NoBackend(
                f"no backend routed for kind {kind!r} — device backends arrive with the L4 era; "
                "oracle execution is spelled: reference(f)(...)"
            )
    ...
    def _check_kind(self, kind: str) -> None:
        if kind not in self.kinds:
            ...
            raise VerifyError(
                f"unknown kind {kind!r} — the kind vocabulary is validated at dispatch "
                f"(known: {known}); register_kind({kind!r}) in the owning package"
            )

    def dispatch(self, target, args: tuple, *, backend: str | None = None):
        """THE hot path. `target` is anything FnType-shaped: Handle or Pipeline.
        ``backend`` is the SPELLED door (reference(f) passes "reference");
        plain calls route by kind and refuse when nothing is routed."""
        if events.SINKS:  # one branch, ~1.2% of a warm hit (120 §5); dark otherwise
            return self._dispatch_traced(target, args, backend=backend)
        self._check_kind(target.kind)
        spec, table = self.specializations, self.table
        be = self.backends[backend] if backend is not None else self.backend_for(target.kind)
        key = (target.fp, tuple(table.fingerprint(a) for a in args), be.fp, spec.generation)
        record = spec.probe(key) or spec.get_or_compile(key, lambda: self._build(target, args, be))
    ...
    def _dispatch_traced(self, target, args: tuple, *, backend: str | None = None):
        ...
        t3 = perf_counter_ns()
        result = record.launch(record.staging, leaves)
        t4 = perf_counter_ns()
        for name, dur in (
    ...
    def extend(self) -> Registry:
        ...
        for a in ("lower_rules", "derived", "routes", "ops", "overloads", "kinds", "compositions"):
            getattr(child, a).update(getattr(self, a))
        child.terminal_kinds |= self.terminal_kinds
        child.dispatcher = self.dispatcher
