...
The dialect now lives in ``pdum/tl/dialect.py``; these are its pins:
the kernel-tier/direct-lowering differential (bit-identical), alignment
as a type rule with source locations, content-keyed lowering, the
per-kind yield protocol, the two-pass refusal, the fold
forward/adjoint differentials between the surviving engines, and the
...
def test_the_dialect_differential_is_bit_identical():
    """The same body, two doors: the kernel tier's cached dispatch vs a
    direct lower_body + run_region — identical bytes."""
...
def _fold_region_with_loss(step, s0, mask):
    """The theorem fold as a region: tl.fold (positional contract) summed
    to a scalar loss; returns (region, names)."""
    from pdum.tl.dialect import region_names

    b = Builder({**CORE_OPS, **TL_OPS})
    p_s, p_src = b.param(0, tensor_type(s0)), b.param(1, tensor_type(mask))
    fold = b.emit("tl.fold", p_s, p_src, regions=(step,), dim="tm", state=("s",), element=("m",), out=("final", 0))
    zloss = b.emit("tl.reduce", fold, f="sum", dims=("x",))
    region = Region(params=(p_s, p_src), body=(b.emit("core.yield", zloss),))
    names = region_names(region, ("s0", "mask"), {id(fold): "sf", id(zloss): "zloss"})
    return region, names


def test_b1_fold_forward_differential():
    """The fold op IS the iterated step: tl.fold under run_region vs the
    explicit host loop over the same step region — identical bytes."""
    ...
    want = s0
    for q in range(TM):
        want = run_region(step, [want, mask.select(tm=q)])
    np.testing.assert_array_equal(got.to_numpy(order=("x",)), want.to_numpy(order=("x",)))


def test_b2_store_all_adjoint_differential():
    """Two surviving engines, one gradient: the per-step VJP under the
    schedules door (fold_grad) ≡ the whole-program region grad of the
    same fold, wrt the initial state."""
    ...
    region, names = _fold_region_with_loss(step, s0, mask)
    rg = grad(region, "zloss", wrt=("s0",), names=names)
    want = run_named(rg.region, {"s0": s0, "mask": mask}, rg.names)[rg.grads["s0"]]
...
def test_step_layernorm_grad_differential():
    """THE C4.3b flagship, region-only: the general region VJP
    (derive_vjp) vs the whole-program grad engine — the same function
    through two doors, gradients wrt x, g, AND b to 1e-12."""
    from pdum.tl.autodiff import grad
    from pdum.tl.dialect import derive_vjp, run_named
    from pdum.tl.lifting import lift_step
    from pdum.tl.zoo.zoo_common import layernorm

    def lnl(x, g, b):
        h = layernorm(x, g, b, feat="d", eps=1e-5)
    ...
    g = Tensor.from_numpy(rng.standard_normal(8), ("d",))
    b = Tensor.from_numpy(rng.standard_normal(8), ("d",))
    ls = lift_step(lnl, x=x.layout, g=g.layout, b=b.layout)
    rg = grad(ls.region, ls.outputs[0], names=ls.names)
    env = run_named(rg.region, {"x": x, "g": g, "b": b}, rg.names)
    # the general region VJP: the same function, a ones seed
    region = lower_body(
        layernorm,
        (tensor_type(x), tensor_type(g), tensor_type(b)),
        kind="step",
...
# The C4.3c migration-view (region -> Program) export tests died with
# export_program at the excavation's close; the adjoint behaviors they
# pinned (max-reduce at-kink, layout-op chains) live in test_autodiff.
