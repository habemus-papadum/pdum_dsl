...
| **routing** | `Registry.routes`: kind → backend name; unrouted kinds use the default. Role vocabularies ship WITH their owning package: the `demo.simple_shader` package registers and routes the DEMO-scoped kinds `simple_shader.compute`/`simple_shader.fragment` — the plain names `compute`/`fragment` are reserved for the real families (080, step 14). `device` (stdlib's, `@jit`'s default) has no route: it falls to the registry default. | ch10 |
| **compute-family contract (v1)** | A `simple_shader.compute` kernel's parameters ARE its thread coordinates (`f(i, j)` per grid point); the call passes the launch DOMAIN — `k(out=(W, H))` — which rides the leaves channel as runtime data. Changing resolution never recompiles. Explicit out-buffers arrive with ndarrays (ch12). | ch10 |
| **narrowing** | A backend layout+text policy, never a kernel concern: WGSL has no f64/i64, so the wgsl backend computes f32/i32 — in the rendered text AND in its `dest_for` (f64 captures pack as 4-byte `<f` uniform slots). Differential tests compare within f32 tolerance. | ch10 |
| **Env struct** | The uniform block the wgsl backend renders from the plan: one member per scalar leaf, named by byte offset (`env.f8`). The uniform layout is the printable plan, not a convention. `@workgroup_size` bakes into the text = the artifact key (block=codegen, grid=runtime — 040 §3b, spec-confirmed). | ch10 |
