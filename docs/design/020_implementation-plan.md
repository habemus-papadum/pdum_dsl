...
## Phase IV — width (steps 11–17; re-sequenced at the 130 fork: grad moved AFTER tensors; each still ends in a chapter + walkthrough)
...
| 13 — seams + `over` (RESEQUENCED 2026-07-13, design 130) | kernel seams per 120's tripwire policy: `Lowerer.context` (kills the five string-keyed doors), `lower_handle` prefix + derived-base re-entry (**transform composition**), `DerivedValue` protocol + one ValueKind, binder discipline; vmap RENAMED `over`; woven-map merge + duplicate-axis refusal | ch13 updated in place (`over`, composition cells) | the transform-plumbing bug class (4 of step-12's 9 findings) dies at the root |
| 14 — tensor values on CPU (design 130 §4, §8.2) — FIRST INSTALLMENT LANDED 2026-07-13: argument arrays + the grid family; THE GATE MET at 16× (ch14-arguments-and-the-grid); remaining: tensor dialect ops, comprehensions, DPS array results, over-emits-map-IR | array RESULTS (DPS out-arrays) + array args; `core.for` kind/axis attrs; `tensor.map/reduce/contract` + named comprehensions lowering to loops on python/C; `over` v2 emits map-loop IR; C gains the in-artifact lane loop | `ch14-tensors-on-cpu`: unbatched attention block, `over`-batched, ≥10× the per-lane spike on C; softmax comprehensions vs numpy | "write high, lower late" proven with zero GPU dependency |
| 15 — grad (was 13; AFTER tensors on purpose: adjoints of `contract`/`reduce` are textbook — differentiating hand-tiled scalar loops is misery) | partial-eval + transpose (~450 lines, own step by design), `custom_vjp` hatch; tangent carries unlock in-body `D` and `D`-through-loop fusion (110 §5) | `ch15-differentiating-a-shader`: grad of a smooth SDF shader; a design-optimization mini-loop (gradient-descend a shape parameter, live) | AD architecture |
| 16 — CUDA + Metal backends (was 14; revised 2026-07-12, 070/R13/R14: OWN both stacks; 130 adds the TILE family: `stage`/`barrier`/tokens, mma-backed `contract`, the GEMM chapter) | `backends/cuda/` on **cuda.core** (opt-in-cache off; escalation: once-per-FastRecord pointer table into staging via `cuLaunchKernel`); `backends/metal/` on **ctypes-objc/PyObjC** (`setBytes`=staging, `setBuffer`=leaves); cupy/MLX demoted to optional fallback runtimes/allocators; CUDA developed design-for-skip + Modal burst (**parallel agents**, one per backend); `raw_kernel` escape hatch | `ch15-four-targets-one-ir` [platform-gated cells]: same kernel on every available target, differentially compared | multi-backend claim at N=4–5 |
| 17 — units (was 15) | `Dim`/`Quantity`, unit aspect column, `Affine` pack converters | `ch16-dimensions-and-units`: mm→inch knob tweak with **zero recompiles** (pack-tier miss only, counters shown); dimension error naming both locs | two-tier law under a real domain |
