...
def test_if_statement_joins_and_stays_lazy():
    ...
    assert f(4.0) == 0.25
    assert f(0.0) == 0.0  # an eager 1.0/x would have raised ZeroDivisionError
...
def test_if_statement_multi_name_join():
    ...
    assert f(1.0) == 3.0
    assert f(0.25) == 0.75  # b keeps its pre-if value on the else path
...
def test_for_loop_single_carry_and_zero_trip():
    ...
    assert make(4)(1.0) == 1.0 + 0 + 1 + 2 + 3
    assert make(0)(7.0) == 7.0  # zero-trip: the carry is the init, defined
...
def test_nested_loops_and_branches_match_python():
    ...
    assert abs(f(0.5) - twin(0.5)) < 1e-12
...
def test_one_sided_names_die_with_their_suite():
    ...
    from pdum.dsl.kernel.lower import NameFateError
